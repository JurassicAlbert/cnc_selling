...
| **Recommended next task** | Finishing **ARCH-02**. **The PR was opened and merged on 2026-09-05, and CI ran for the first time - red, on its first database step (CI-01).** That is fixed and the whole pipeline reproduced locally against virgin databases, but `main` carries the red run until the fix is merged. |
...
|---|---|---|---|---|
| 1 | **ARCH-02, the rest** | P2 | `SummaryStep` (189 lines) and five smaller components still sit in `Configurator.tsx`. The same mechanical move as the four already out. | none |
