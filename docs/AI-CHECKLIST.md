...
- [ ] **PERF-02 · P1 · performance — Every `generateMetadata` route queries the same row twice**
  - **Files:** the five pages with `generateMetadata` (`produkt/[slug]`, `[category]`, `kolekcje/[slug]`, `blog/[slug]`, `strony/[slug]`)
  - **Current:** the same repository function runs in both `generateMetadata` and the page body. Prisma is not deduped by Next; `React.cache()` appears nowhere in the codebase.
  - **Expected:** wrap those five functions — and `getSession` (called twice per render, once by the chrome and once by the page) — in `cache()` from `react`. Six one-line changes.
  - **Acceptance:** one request to `/produkt/[slug]` issues one product query and one session lookup.
  - **Evidence:** `REVIEW-PERFORMANCE.md` Findings 2 and 3
  - **Status:** TODO
