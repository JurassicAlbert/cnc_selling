...
| **Overall implementation status** | Feature-complete against `ARCHITECTURE.md` P0–P9. All P0s closed, **every listed product can actually be configured and ordered** (T-16 holds the line), §16.1's header half exists, and there is CI. **The e2e suite is green for the first time (56/56, both browsers)** - see T-23. Remaining: 0 P0, 1 P1, 33 P2, 14 P3. |
| **Current highest-priority issue** | **ADMIN-01** - 66 real orders are unreachable in the panel today. |
| **Recommended next task** | **UX-23** (owner request, and its stated prerequisite UX-21 is now done), then **ADMIN-01**. **Still outstanding: open the PR and watch the CI workflow run** - the branch is pushed but the workflow has never executed on GitHub. It has a much better chance of being green than it did: T-23 fixed four reasons it could not have been. |
...
| **Last completed area** | **UX-21, and SEC-11 + T-23 found while verifying it** (2026-09-04). Before them: the warehouse tool, the burger nav, the em-dash sweep, PERF-02 + PERF-05, BUG-05, BUG-06, BUG-07, SEC-04, SEC-10, ARCH-01, SEC-05, SEC-01, SEC-02, SEC-03, BUG-02, BUG-03, BUG-24, BUG-35 and the carried-forward P1-8. |
| **Ready for the next implementation step?** | **Yes.** 1082/1082 unit + integration tests, **56/56 e2e across both browser projects**, typecheck clean, lint clean, production build succeeds. |
...
- [x] **UX-21 · P2 · UX/UI - the configurator showed a price for an option it will then refuse** - **DONE 2026-09-04.**
  ...
  - **Status:** DONE
- [ ] **UX-23 · P2 · UX/UI - rework the cart, the navbar and the search bar towards the Bazaar layout** - owner request, 2026-09-04.
  - **Reference:** `https://template.getbazaar.io/cart`. The owner asked that the **cart/basket**, the **navbar**, the **topbar above the navbar**, the **search bar**, and the **category rail shown on the cart view** resemble that layout.
  - **What to take, and what not to.** Take the *arrangement*: a slim topbar above the main nav, a search field that owns the centre of the header with a category selector attached to it, and a cart page laid out as a list of line cards beside a sticky summary panel. Do not copy the template's CSS, assets, typography or colour: this shop has its own visual identity (ARCHITECTURE.md §2.1, and the owner's standing instruction to preserve the existing background and decorative design), and lifting a commercial template's styling wholesale is both an identity problem and a licensing one.
  - **Constraints that already apply and must not be lost:** the storefront header is a Server Component with zero client JS and MUI is lint-banned from `(marketing)`/`(shop)` (§2.1, measured at 3.8s LCP when it was not). The burger added on 2026-09-04 must keep working, and its failure direction must stay "show everything" rather than "show nothing". No em-dash in any new copy.
  - **Sequencing note:** UX-21 is done (2026-09-04), so the prerequisite is cleared. This is the next UX item.
  - **Status:** TODO
...
| 3 | **UX-23** | P2 | Owner request, 2026-09-04: the Bazaar-shaped cart, navbar, topbar and search bar. Its stated prerequisite (UX-21) is now done. | none |
