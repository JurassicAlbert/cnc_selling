...
| **Current highest-priority issue** | **BUG-04 + BUG-19** - the highest-priority remaining items; nothing is P0 or P1 any more. |
| **Recommended next task** | **BUG-04 + BUG-19**. **Still outstanding: open the PR and watch the CI workflow run** - the branch is pushed but the workflow has never executed on GitHub. It has a much better chance of being green than it did: T-23 fixed four reasons it could not have been. |
