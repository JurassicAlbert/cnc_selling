...
Started as vertical slices, same discipline as P7b (§16A.6) — slice 1 (global search), slice 2 (`@mui/x-data-grid` adoption, starting with Orders), and slice 3 (`DataGrid` on the six catalogue list pages — Kategorie, Produkty, Materiały, Wykończenia, Wzory, Kolekcje) shipped 2026-08-27; the remaining ~9 list pages (Klienci, Opinie, FAQ, Strony, Produkcja, Personel, Szablony e-mail, Dziennik zdarzeń, Weryfikacja) and the rest of the 23-item list are still open, picked one at a time.
