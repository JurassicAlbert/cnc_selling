...
Started as vertical slices, same discipline as P7b (§16A.6) — slice 1 (global search), slice 2 (`@mui/x-data-grid` adoption, starting with Orders), slice 3 (`DataGrid` on the six catalogue list pages — Kategorie, Produkty, Materiały, Wykończenia, Wzory, Kolekcje), and slice 4 (`DataGrid` on the four remaining plain navigate-to-detail lists — Klienci, FAQ, Strony, Weryfikacja) all shipped 2026-08-27. Five list pages deliberately still use a plain `<Table>`, not deferred by oversight: Opinie and Personel have per-row action buttons (approve/reject, revoke) with no natural row-click target, a genuinely different pattern from "click a row to navigate"; Produkcja's rows link out to a *different* entity's detail page (an order), not their own; Szablony e-mail is a fixed 2-row list where sorting/pagination/density add no value; Dziennik zdarzeń's diff column holds variable-height pretty-printed JSON that doesn't suit `DataGrid`'s fixed-row-height model. Each is its own future slice with its own design, not a mechanical repeat of `EntityDataGrid`. The rest of the 23-item UX-polish list is still open, picked one at a time.
