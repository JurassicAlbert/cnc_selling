...
- [~] CSV import/export on catalogue tables — **export already worked** (`GridToolbar`'s built-in "Pobierz jako plik CSV", live on all 13 admin `DataGrid`s since P7c, no extra code). **Import built 2026-08-27 for Categories** (`applyImportCategoriesFromCsv`/new `CsvImportForm` primitive, `/panel/kategorie`) — a real pilot proving the pattern: every row goes through the exact same audited `applyCreateCategory` a manual create does, per-row success/failure reported rather than an all-or-nothing batch, real Polish-diacritic round-trip tested. The other 5 catalogue entities don't have the import form wired yet — same `CsvImportForm` primitive, needs each entity's own `applyImportXFromCsv`, a mechanical follow-up (see `docs/HANDOVER.md` §9z31)
