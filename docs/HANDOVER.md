...
## 9z26. "Preview as customer" — Product half — 2026-08-27

Continuing autonomously: `ARCHITECTURE.md` §16A.5's own wording is specific — "open any product or design in the storefront exactly as a customer sees it, one click from the edit form." Built the Product half; the Design half needs a real deep-link into the configurator (no standalone design page exists to open), a genuinely separate, more involved slice — deferred and documented, not silently dropped.

### The real design constraint: reuse the live page, not a clone

"Exactly as a customer sees it" only means something if it's the *same* page rendering, not a parallel admin-maintained copy that drifts the moment one of them changes. So this isn't a new route — `/produkt/[slug]/page.tsx` itself gained a `?podglad=1` query flag, checked against a real server-side session (`getSession().role === 'STAFF' | 'ADMIN'`) before it does anything: the flag alone grants nothing, and a non-staff visitor (or a staff member without the flag) still gets the same 404-on-inactive behavior as before. A visible banner (`productPreviewBannerPl`) makes it unmistakable this is a preview render, and the real `product_view` analytics event is skipped for it — a staff click-through must never inflate real traffic numbers.

### Two real, pre-existing bugs this feature's own live verification surfaced and fixed

Neither was hypothetical — both reproduced immediately on the first real click-through of a genuinely inactive product, not found by inspection:

1. **The Configurator's own URL-sync effect was clobbering the whole query string, not just its own keys.** `Configurator.tsx`'s per-selection-change effect calls `router.replace()` to keep the URL shareable — but it built that URL from scratch every time (`writeSelectionsToSearch(selections)`), silently discarding anything else already in the query string. `?podglad=1` vanished the instant the configurator's effect first ran (even with zero selections made), which meant an inactive product 404'd a moment after its own preview page had just rendered correctly via SSR. Fixed by explicitly re-reading and re-attaching `podglad` from the current URL on every replace, rather than a wholesale rewrite. (This same clobbering already silently dropped the pre-existing `?edit=...` cart-editing param too, on every keystroke after the first mount — invisible until now because nothing ever needed `edit` to survive past its one-time initial read.)
2. **`getConfiguratorSnapshot` (the Server Action every configurator selection change calls) had no awareness of preview mode at all**, even after fix #1 kept the page's own URL intact — it always called `getConfiguratorProductData(slug)` with its default `activeOnly: true`, so the configurator step machine itself still couldn't find an inactive product and rendered "Nie znaleziono takiego produktu." inline, even though the page around it was correctly in preview mode. Fixed by giving `getConfiguratorProductData` an `activeOnly` parameter (defaults to `true` — every existing call site's behavior is unchanged) and threading a `isPreview` flag from `ProductPage` → `<Configurator isPreview>` → `getConfiguratorSnapshot(..., isPreview)`, which **re-verifies the staff session server-side itself** rather than trusting the client-passed boolean — the same "the flag alone grants nothing" discipline as the page's own gate, since a Server Action call has no other proof of who's asking.

### Verified

`npm run typecheck && npm run lint && npm test && npm run build` all clean (591/591 — the previously-flaky `upload.test.ts` 5MB-SVG test passed this run too, confirming §9z25's diagnosis; 4 new tests in `product-preview.test.ts` covering `getActiveProductBySlug`/`getProductBySlugForPreview`'s active/inactive/nonexistent cases — `getConfiguratorProductData`'s new flag has no dedicated automated test, honestly: no existing integration test builds a full configurator fixture (Product+Material+MachineSettings+PricingSettings), and building one from scratch was disproportionate to this slice; it's covered by the live-browser verification below instead, against the real seeded dev DB). Restarted the dev server before live-verifying, per §9z2's standing rule — the first attempt (before restarting) showed real console errors (`useEffect` deps-array-size-changed warning, a 500) that were confirmed to be stale-HMR artifacts from live-editing `Configurator.tsx`/`configurator.ts` mid-session, not real bugs — they were gone on a fresh server + fresh tab. Live-verified end to end: deactivated the real seeded `Stołek loftowy z grawerem` product → confirmed `/produkt/stolek-loftowy-z-grawerem` (no flag) 404s even while logged in as staff → confirmed `?podglad=1` renders the full page with the preview banner, the real configurator (all 7 steps), and the real assigned design → clicked an actual design option and confirmed `podglad=1` survived in the URL alongside the real `d=...` selection param, with zero console errors → reactivated the product afterward via the same real toggle mechanism, restoring its exact prior state (audit trail shows both the deactivate and reactivate, honestly, not hidden).

## 9z27. Cross-cutting Polish-correctness verification sweep — 2026-08-27

Continuing autonomously through `docs/CHECKLIST.md`'s "Cross-cutting verification" section — most of its Polish-locale-correctness lines had been sitting unchecked, but this codebase has built real infrastructure for most of them since P1. Rather than assume, verified each one against actual behavior (running real `Intl` output, reading the real implementation, checking real test coverage) and only checked off what held up — this surfaced one genuine, real gap, fixed below, and one line whose literal example was wrong for a reason worth recording rather than silently "fixing" the code to match a wrong example.

### The one real gap: `comparePl`/`sortByPl` existed but were never actually used

`domain/text/collation.ts`'s Polish-collation comparator (`ą` sorts next to `a`, not after `z`) has existed since P1, real and unit-testable — but grepping every real call site found zero usages anywhere outside its own file. Every list in the app is ordered by a staff-curated `sortOrder` or a `code`, never alphabetically by name at the DB level, so the gap was invisible in every page render — until a user actually clicks a "Nazwa"/"Tytuł" column header in one of the 13 admin `DataGrid`s to sort alphabetically, which uses MUI's own default string comparator (plain code-point order, exactly the "Zebra before Ćma" bug the module's own doc comment describes). Fixed by adding `sortComparator: comparePl` to every genuinely-Polish-text column (name/title/question/author fields) across all 13 grids — `CategoriesDataGrid`, `ProductsDataGrid` (two columns: name and category name), `MaterialsDataGrid`, `FinishesDataGrid`, `DesignsDataGrid`, `CollectionsDataGrid`, `StaticPagesDataGrid`, `FaqDataGrid`, `BlogPostsDataGrid`, `CustomersDataGrid`, `OpinieDataGrid`, `StaffDataGrid`, `OrdersDataGrid`. New `tests/unit/collation.test.ts` (the module's first-ever test file) proves the exact diacritic-ordering claim directly (`['Zebra','Ćma','Łoś'].sort(comparePl)` → `['Ćma','Łoś','Zebra']`), plus `foldPl`/`matchesPl` coverage.

### A checklist example that was itself slightly wrong, documented rather than "fixed"

The checklist's own currency example, `1 234,56 zł`, doesn't actually render that way — real Polish CLDR only inserts the thousands-grouping space from 5 digits (`minimumGroupingDigits: 2`), so a 4-digit amount like `1234.56` correctly renders `1234,56 zł`, no space, per genuine `Intl.NumberFormat('pl-PL')` output (verified directly, not assumed) — and `formatPln()`'s own doc comment already recorded this exact nuance. Checked the line off with that caveat recorded rather than either silently accepting a technically-wrong example or "fixing" already-correct real locale behavior to match it.

### Also verified true, no code change needed

`latin-ext` font subset (`ui/theme/fonts.ts`), genitive-form dates (`Intl.DateTimeFormat({dateStyle:'long'})` on the blog's 3 date-rendering spots), comma-decimal numeric parsing (`parseDecimalPl`, already tested), diacritic-insensitive search (`matchesPl`, already wired into the real storefront product search), the real NIP-checksum/postal-code/phone validators (`domain/checkout/validate.ts`, 8 existing tests) — and slug safety: no auto-transliteration UX exists, but every `applyCreateX` action's `SLUG_PATTERN` regex rejects any diacritic outright, so a percent-encoded or diacritic slug can never be created in the first place, satisfying the requirement's actual intent even without the "convenience" half.

### Left honestly unchecked, not guessed at

Polish plural forms (no pluralization helper found — a real, unaudited gap), Polish quotation marks and single-letter-word line-break rules in copy (would need a full content-file sweep, not done this pass), engraving-font cmap/preview-parity items (P4-scoped, out of reach of this sweep).
...
## 9z29. Real confirmation dialogs for irreversible actions — 2026-08-27

Continuing autonomously through `docs/CHECKLIST.md`. `PricingSimulator.tsx`'s own doc comment already flagged this honestly — its publish button used a bare `window.confirm()` "since no MUI Dialog-based confirmation pattern exists anywhere in this codebase yet." Grepped for `Dialog`/`window.confirm` to check the rest of the panel and found two other genuinely-terminal actions (order cancellation, customer anonymization) had **no confirmation at all** — a single misclick submitted them immediately.

### One shared `ConfirmSubmitButton`, two modes

New `src/ui/primitives/ConfirmSubmitButton.tsx` — a real MUI `Dialog` (title, message, Cancel/Confirm buttons), not a native browser prompt. Two call shapes, because the three real call sites needed both:

- **`formId` mode** (order cancellation, customer anonymization) — keeps the existing real `<form action={serverAction}>` submission entirely intact. The trigger button is `type="button"` (opens the dialog, submits nothing); the dialog's own confirm button is `type="submit" form={formId}` — the plain HTML mechanism for associating a submit button with a form it isn't a DOM descendant of, needed because MUI's `Dialog` portals to `document.body` and is never actually nested inside the `<form>` element it needs to submit. `useFormStatus()` still correctly finds the right form's pending state because it's called from the *trigger* button's own position in the JSX tree (inline inside the form), not from inside the portaled dialog content.
- **`onConfirm` mode** (pricing publish) — for the one call site that doesn't use a plain form submission at all; `PricingSimulator` already drives its own client-side pending/error state around a direct Server Action call (needed for the `router.push` on success), so this mode takes a plain callback and an explicit `pending` prop instead.

### The two previously-unguarded actions, both genuinely terminal

Confirmed via the domain layer before wiring anything, not assumed: `domain/order-status/transitions.ts` has `CANCELLED` in `TERMINAL_STATUSES` with the graph's own comment "no edges leave them"; customer anonymization has no undo path anywhere in `admin-customers.ts`. Order cancellation's `ConfirmSubmitButton` is also now disabled until the (server-mandatory) note field has real content — a small, free UX improvement alongside the dialog itself, matching `CustomerAnonymizeForm`'s own existing note-required pattern rather than letting a client submit and only then learn from a server error that the note was required.
...
## 9z35. CSV import for Collections and Products — 2026-08-28

Continuing autonomously: §9z31 built CSV import as a real, complete pilot on Categories, explicitly flagging the other 5 catalogue entities as "a mechanical follow-up." Checked before assuming the whole rest was mechanical — it wasn't uniform.

### Two entities really are mechanical: Collections

`DesignCollection` has the same shape as `Category` — no required file upload, no foreign key. `applyImportCollectionsFromCsv`/`importCollectionsFromCsv` is close to a direct copy of the Category version (`slug,namePl,descPl,sortOrder`), wired into `/panel/kolekcje` the same way.

### Products needed one real new piece: resolving a foreign key from a CSV cell

`Product.categoryId` is required and a real FK. The CSV carries a human-readable `categorySlug` column instead of an opaque id — a staff member editing a spreadsheet has no reason to know a cuid — and `applyImportProductsFromCsv` resolves it with `prisma.category.findUnique({where: {slug}})` per row, failing just that row (not the batch) when the slug doesn't exist. Also new: `Product` has 8 required numeric columns (prices, day counts, dimensions) and 2 booleans. A missing or non-numeric required column fails that row with a specific message (`csvRequiredInt` returns `null` rather than silently coercing to 0) — a silent `0` would create a real, broken product (a "0 zł" wall-art listing) that the import report would show as a *success*, which is worse than a rejection. Booleans accept `true`/`1`/`tak` (Polish spreadsheet users would naturally type `tak`), anything else is `false`.

### Deliberately not attempted: Materials, Finishes, Designs

All three take `FormData` with a mandatory uploaded image file in their real `applyCreateX` (`applyCreateMaterial`/`applyCreateFinish`/`applyCreateDesign`) — a flat CSV row has no file to attach. Forcing them through the same `CsvImportForm` pattern would mean either quietly making the image optional for imported rows (a real product-quality regression: an unavailable-looking blank thumbnail in the catalogue) or inventing a second input mechanism (an image-URL column, a zip+CSV combo) that's a genuinely different feature, not a copy-paste of this one. Left unbuilt and said so, rather than shipped half-right.

### Verified

`npm run typecheck && npm run lint && npm test && npm run build` all clean (614/614 — 4 new tests: Collections' valid-row-plus-bad-row case, Products' categorySlug-resolves case, Products' unknown-categorySlug-fails-just-that-row case, Products' non-numeric-required-column-fails case). Restarted the dev server before live-verifying, per §9z2's standing rule. Live-verified both imports end to end with a **real click-through file upload**, not just "the form renders" — this browser tool cannot drive a native OS file picker (documented in §9z31), but a `DataTransfer`-constructed `File` assigned to the input's `.files` plus a dispatched `change` event is the standard, legitimate way test automation drives file inputs without one, and it exercises the exact same `<form action={formAction}>` → Server Action → `Papa.parse` path a real upload would. Collections: imported a row, "Zaimportowano 1 wiersz" appeared, the new "Weryfikacja CSV" collection showed up in the real grid with `sortOrder: 9`. Products: imported a row with `categorySlug: "loft"`, confirmed via a direct `fetch('/panel/produkty')` that the new product actually renders in the real list (not just a client-side illusion). Both verification rows removed afterward via a direct script (manually-created browser-test data, not the e2e-disposable-account convention, so cleaned up rather than left — matching the precedent already set for other manual browser-test artifacts this session, e.g. §9z19's temporary pricing draft).

## 9z36. Validation messages name the fix, not the rule — and a real bug found live along the way — 2026-08-28

Continuing autonomously through `ARCHITECTURE.md` §1192's own line: „Minimalna szerokość dla dębu to 1,2 mm; ten wzór ma 0,8 mm" beats „Nieprawidłowa wartość". Audited before writing anything: the domain layer's own customer-facing messages (`dimensionMessage`/`feasibilityMessage` in `content/pl/messages.ts`) already do this correctly and consistently — every domain "finding" already carries a real `params` bag threaded from the domain layer's own comparisons, e.g. `WIDTH_ABOVE_MAX` already names the actual limit in millimetres. Nothing to fix there; confirmed, not rebuilt.

### The one real customer-facing gap: `uploadErrorMessage`

Every other `UploadErrorCode` case is already fully self-explanatory from its own fixed text (`'Nieobsługiwany format pliku. Akceptujemy JPG, PNG, SVG i PDF.'` — there's no "real number" to name for an unsupported format). `FILE_TOO_LARGE` was the one exception with an actual actionable number to show and didn't: `'Plik jest za duży.'`, no size, no limit — even though `inspectUploadedFile` already computes both `bytes.length` and `maxBytes` right at the point of the check, just to throw them away. Threaded a `params?: Record<string, number>` field through `InspectFileResult`'s failure branch → `UploadCustomDesignResult`'s failure branch → `uploadErrorMessage(code, params)`, following the exact same "params bag from a domain/inspection layer, formatted by the content layer" shape already used everywhere else. Now reads: "Plik ma 25,5 MB — maksymalny dopuszczalny rozmiar to 25 MB. Zmniejsz plik i spróbuj ponownie."

### A second, more serious bug — found live, not in the diff

Verifying the new message in the browser (a 25.5MB file, comfortably under the app's real 25MB→26MB buffer) worked immediately. Testing the boundary itself — a 26MB file — did not: the "Prześlij projekt" button hung on "Przesyłanie..." forever. `read_network_requests` showed a `500`; `preview_logs` showed why: `Error: Body exceeded 26mb limit` — `next.config`'s own `serverActions.bodySizeLimit: '26mb'` (a deliberate 1MB buffer above the app's real 25MB cap, so the app's own friendly check is normally what fires) was itself exceeded once real multipart overhead was added to a file already at the app's boundary. At that point Next.js rejects the request at the framework level, before `uploadCustomDesign` ever runs — the call throws instead of resolving `{ok: false}`, and `CustomUploadStep`'s `handleSubmit` had no `try/catch` around it. `pending` never got reset; the real error was an `Uncaught (in promise)` visible only in the browser console, never shown to the customer — worse than the generic message this pass set out to fix, since it isn't a message at all.

Fixed with a `try/catch` around the Server Action call: on catch, show the exact same real-numbers `FILE_TOO_LARGE` message using `file.size` (already known client-side, no server round-trip needed) and `maxUploadSizeBytes(file.type)` (the same pure domain function already imported for `UploadWarning`, safe to call client-side — it's math, not I/O). Re-verified on a fresh dev server + fresh tab: the same 26MB file now resolves immediately with the real message and a re-enabled button, no unhandled rejection in the console.

### Admin-side sweep

Grepped every `src/server/actions/*.ts` file for `musi być`/`nie może` cross-field and numeric-bound messages and rewrote the ones stating an abstract rule without the actual submitted values — 8 messages across `admin-products.ts` (min/max width, min/max height, min/max price, min/max production days — the width/height check was previously one combined `||` condition that couldn't say *which* dimension was wrong; split into two so it can), `admin-materials.ts` (non-positive price, converted back to zł for display since the stored value is grosze), `admin-finishes.ts` (min/max extra days), `admin-designs.ts` (detail-level range), `admin-pricing.ts` (which specific rate failed — previously one combined check over all three rates with no way to tell which — plus VAT and packaging-tier-price, each now showing the actual submitted number), and `reviews.ts` (rating range). Not touched: the many `'X jest wymagane'` ("X is required") messages across every action file — those already name the specific field, which is itself naming the fix, not the abstract rule.
...
## 9z43. Phase 2 shipped: pattern/design library — a standalone, reusable saved-designs page

Continuing the plan from §9z42. The `CUSTOM_UPLOAD` configurator step (the thing the owner's prompt called a "pattern generator") is now backed by a real, standalone customer page instead of being reachable only mid-checkout on one product.

- Migration adds `CustomerDesign.titlePl String?` (the customer's own label — optional, older rows/quick re-uploads have none) and `Design.featured Boolean @default(false)` (an admin-curated highlight for the real pattern-browsing page later, purely presentational).
- New `src/server/repositories/customer-designs.ts`: `listOwnedCustomerDesigns(owner)` — the one genuinely new, directly-testable unit this phase added (pure, takes `Owner` explicitly, mirrors the `find*(owner)`/`require*()` split `design-review.ts` already established). `uploadCustomDesign` extended to accept an optional `titlePl`, unchanged otherwise — backward compatible, since the Configurator's own inline upload still sends none.
- New page **`/moje-konto/wzory`**: lists every `CustomerDesign` the current owner (logged-in user or guest session — the guest cookie is deliberately kept alive across login specifically so pre-login uploads stay reachable, per `ownership.ts`'s existing design) has ever uploaded, each with its title-or-filename, real review status, a thumbnail via the existing `/api/plik/[fileId]?preview=1` route, and upload date — plus a real MUI upload form (`CustomerDesignUploadForm.tsx`) that reuses `uploadCustomDesign` unchanged, just with the new title field. New nav link in the account layout.
- The `CUSTOM` product's own configurator upload step (`CustomUploadStep` in `Configurator.tsx`) gained a "reuse a saved design" picker — shown only when the owner actually has ≥1 saved design, letting them skip re-uploading. Caught and fixed a real correctness bug in this before it shipped: naively wiring the picker into the existing "just uploaded" success branch would have shown a hardcoded "pending review" message even for a design that was already `APPROVED` — fixed by looking the reused design up in `savedDesigns` and branching the displayed status/message on its *real* status.
- Admin `Design` form gained the `featured` checkbox. `applyDuplicateDesign` builds its `DesignFields` object literal by hand (doesn't go through `readDesignFields()`) — adding a new required field to the type surfaced a real missing-field TypeScript error there, fixed by explicitly setting `featured: false` on the copy (a curated highlight needs deliberate re-review, not free inheritance) before running typecheck; typecheck came back clean on the first run after, confirming it was the only spot that needed it.

### Verified

New `tests/integration/customer-designs.test.ts` (5 cases: session-token ownership, userId ownership, cross-owner isolation, the real "no owner → empty list, not everything" short-circuit, `hasPreview`/newest-first ordering) and 4 new cases added to `tests/integration/admin-designs.test.ts` for `featured` (create default/explicit, update toggle, never inherited by duplicate). Full suite 630/630, `typecheck`/`lint`/`build` all clean. Dev server restarted clean (no repeat of §9z42's post-rebuild `/panel/*` 404 flake this time).

Live-verified all three changed surfaces by seeding one real `CustomerDesign` row directly in the dev DB for the logged-in preview session, then removing it afterward:
- `/moje-konto/wzory` lists it (title + original filename).
- The `wlasny-projekt-z-grawerem` product page's server-rendered `savedDesigns` prop carries the real row through to the Configurator (id/fileId/titlePl/status/mimeType/hasPreview all correct).
- `/panel/wzory/nowy` renders the new "Wzór wyróżniony" checkbox.

Verification method matters here: this route tree has a `loading.tsx`, so it streams. This harness's `read_page`/`get_page_text`/live-DOM `javascript_tool` reads got stuck showing the `Ładowanie…` fallback forever, with the real resolved content sitting unswapped in a `hidden` staging `<div>` at the end of `<body>` — reproduced identically on an untouched, already-shipped sibling route (`/moje-konto/projekty`), so it's a harness limitation, not a regression. Confirmed the real, correct final output instead via `fetch(url).then(r => r.text())` executed *from inside the page* (same-origin, cookies automatic) and via a plain `curl` of the same URL — both show the fully-resolved, correct HTML. Documented as browser-tooling quirk #4 in memory (`feedback_browser_tooling_quirks.md`) so a future streamed-route verification doesn't re-diagnose this as a real bug.

## 9z44. Phase 3 shipped: external pattern resources + a real public `/wzory` page

Continuing the plan from §9z42/§9z43.

- New `ExternalPatternResource` model, admin CRUD at `/panel/zasoby-zewnetrzne` built by mirroring the FAQ admin CRUD files almost line-for-line (repository, actions, form, data grid, three pages) — the exact "reuse the existing pattern, don't invent a new one" instruction from the owner's own prompt (§12/§18). `url` is server-validated as a real parseable `http(s)://` link before it's ever stored, since it's only ever rendered as a real `<a href target="_blank">` — a `javascript:` URI or garbage string is rejected outright, not just discouraged.
- The plan's own Phase 3 text assumed a "patterns/designs browsing area" would already exist by this point; inspecting the codebase (again, before writing code) confirmed it didn't — `Design` rows were only ever selectable from inside one specific product's configurator, never browsable on their own. Rather than bolt the external-resources section onto an unrelated page, built the real thing implied by both this gap and by Phase 2's own `Design.featured` field (added specifically for "the real pattern-browsing page," per its own code comment at the time): a new public `/wzory` page — an honest gallery, not a checkout entry point (no `/wzory/[slug]` detail route exists, so designs aren't linked anywhere from here) — listing active, rights-clear designs (same `rightsStatus` filter §12 already established, featured-first) with the external-resources section clearly labelled "zasób zewnętrzny" beneath it. New footer link.
- Seeded one real row: 3axis.co, described honestly as an independent third-party resource with its own licensing terms, not vetted or curated content of ours — create-only, matched by `url`.

### Verified

New `tests/integration/admin-external-pattern-resources.test.ts` (9 cases, including the URL-scheme rejection) and `tests/integration/designs-public.test.ts` (4 cases, including the featured-first ordering). Full suite 643/643, `typecheck`/`lint`/`build` all clean. Dev server restarted clean. Live-verified: `/wzory`'s external link carries the real `href`/`target="_blank"`/`rel="noopener noreferrer"` (confirmed via `javascript_tool` reading the actual anchor element's properties, not just its markup); the admin list/new/detail pages all render correctly (confirmed via server-rendered `fetch` output for the detail page, since the interactive click-through tools require the Browser pane to be actually displayed/foregrounded to resolve element refs and take screenshots, which it wasn't this pass — not a limitation specific to this feature, and the same mutation logic is already covered by the 9 integration test cases at the same rigor as every other CRUD entity this session built).

## 9z45. Phase 4 shipped: product collections — a new model, kept clean of the existing one

Continuing the plan from §9z42–§9z44.

- New `ProductCollection` + `ProductCollectionItem` (many-to-many, same shape as `ProductDesign`) — deliberately a separate model from `DesignCollection`, which already existed and groups *patterns* (§9z42 flagged this incompatibility before any code was written). What wasn't flagged until this phase's own inspection: `/panel/kolekcje` was already the live admin URL for `DesignCollection`, and `DesignCollection`'s own admin components are literally named `CollectionForm.tsx`/`CollectionsDataGrid.tsx`. Building the new feature at the same URL or with colliding component names would have broken the existing one silently. Resolved by giving the new feature its own namespace throughout: `/panel/kolekcje-produktow`, `ProductCollectionForm.tsx`/`ProductCollectionDataGrid.tsx`/`ProductCollectionItemsEditor.tsx`, `admin-product-collections.ts` for both the repository and the actions file — `DesignCollection`'s files and routes were never touched.
- Admin CRUD mirrors the FAQ/external-resources pattern; the product-assignment nested editor mirrors `DesignAssignEditor.tsx` (design→product) in reverse (product→collection), reusing the exact upsert/remove/`available.filter(...)` shape rather than inventing a new one — the owner's own "reuse the existing architecture" instruction (§18) taken literally a third time this pass.
- New public `/kolekcje` + `/kolekcje/[slug]`, reusing `ProductCard`/`Breadcrumbs` exactly as the category page does — same card component, same visual language, so a collection reads as "part of the shop," not a bolted-on separate section. Copy on both pages explicitly frames a collection as ready-made/independently-created, not customer-request-driven (§14's own distinction). Seeded one real row ("Polecane na prezent") with two real already-seeded products.

### Verified

New `tests/integration/admin-product-collections.test.ts` (8 cases, including that assigning/removing a product has the correct effect on both the admin item list and the real public product listing). Full suite 651/651, `typecheck`/`lint`/`build` all clean. Dev server restarted clean. Live-verified: both public pages render the real seeded content with working `ProductCard`s and correct breadcrumbs; the admin list and detail pages render correctly, with the sidebar showing "Kolekcje" and "Kolekcje produktów" as two clearly distinct entries (confirmed by reading the actual rendered nav, not just checking for route collisions statically) and the nested items editor correctly reflecting real assign/remove state.

## 9z46. Phase 5 shipped: delivery method CRUD + real checkout wiring

Continuing the plan from §9z42–§9z45. The biggest phase so far — the first to touch the money-critical `create-order.ts` path and the first to convert an existing raw-HTML form to real MUI.

- New `DeliveryMethod` model + admin CRUD at `/panel/dostawa`, mirroring `admin-finishes.ts`. `Order.deliveryMethodId`/`deliveryMethodNamePl` added, the name snapshotted the same way `OrderItem.snapshot` already protects against a later catalogue edit silently rewriting history. Pre-existing orders backfilled with an honest generic label rather than a fabricated specific one.
- `CheckoutForm.tsx` — the plan's own text called this out explicitly ("mount ThemeRegistry on checkout per phase 1/2 precedent") — converted wholesale from raw `<input>`/`<fieldset>` to real MUI, not just the new delivery field, so the form doesn't end up half-raw/half-polished. Added a live subtotal/shipping/total estimate that updates as the customer picks a delivery method — deliberately labelled internally as "display-only, `createOrder` is still the only source of truth," since §15.3's "never trust client-side prices" rule is easy to accidentally violate the moment a client-side total display exists.
- `createOrder` now re-validates the chosen delivery method server-side (must exist and be active) before doing anything else, returning a new `DELIVERY_METHOD_INVALID` result if not — checked right after the cart-emptiness check, before the expensive per-item price-revalidation loop.
- **A real, build-breaking client/server import-boundary bug, caught by `npm run build`, not by `typecheck` or `lint`:** `CheckoutForm.tsx` (a `'use client'` component) imported `computeShippingGrosze` as a value from `server/repositories/delivery-methods.ts`. That file's `prisma` import — and the real `pg` Postgres driver behind it — got pulled into the browser bundle, and Turbopack failed the production build outright: `Module not found: Can't resolve 'net'/'tls'/'util/types'` (Node built-ins `pg`'s TCP/TLS socket code needs, which don't exist in a browser). `type`-only imports from a server file are erased and safe; a *value* import isn't, because the bundler can't prove the imported module has no side effects (a module-level `new PrismaClient()`-style side effect is exactly why it can't). Fixed at the root, not patched around: moved the pure `computeShippingGrosze` function into a new `src/domain/checkout/delivery.ts` with zero `prisma`/`pg` dependency — both `create-order.ts` (server) and `CheckoutForm.tsx` (client) now import the same function from there, so the two can never silently drift, and nothing server-only ever reaches the client bundle. This is a real, generalizable lesson for the rest of this plan (phases 6–9 all touch checkout too): a value import from `server/repositories/*` or `server/actions/*` into any `'use client'` file is a latent build break, not just a style smell — `type`-only imports from those same files stay completely safe.
- A **real, honest** decision about `StoreSettings.shippingFlatRateGrosze`: now genuinely unused at checkout, but NOT deleted — a destructive column/UI removal wasn't worth the risk for this pass. Instead relabelled in the admin form ("— nieużywana", with helper text pointing to the new delivery-methods section) so an admin editing it isn't misled into thinking it still does something.
- Seeded two real, honest delivery methods — "Kurier" (a real price, a real free-shipping threshold) and "Odbiór osobisty" (free, manually arranged) — both `trackingAvailable: false`, since no real carrier API exists yet (that's Phase 7's honest-manual-tracking territory, not this phase's to fake).

### A real regression the full test suite caught before it could ship

Making `Order.deliveryMethodNamePl` a required field broke 11 pre-existing `prisma.order.create()` call sites across 10 other integration test files (`admin-orders.test.ts`, `soft-delete-invariant.test.ts`, `admin-pricing.test.ts`, `admin-dashboard.test.ts`, `admin-global-search.test.ts`, `admin-customers.test.ts`, `admin-production.test.ts`, `reviews.test.ts`, `admin-design-review.test.ts`, `auth.test.ts` ×2) — every one of them constructs a test `Order` directly via Prisma, none through `createOrder` itself. `npm run typecheck` and `npm run lint` both stayed clean (Prisma's own generated types don't get checked by either), so only actually running the full suite surfaced this — exactly the reason this session runs `npm test` in full after every phase rather than trusting typecheck/lint alone. Fixed by adding `deliveryMethodNamePl: 'Test'` at each of the 11 call sites (via a single targeted `sed` pass, verified against `git diff` before trusting it), not by weakening the new required field back to optional.

### Verified

New `tests/integration/delivery-methods.test.ts` (5), `admin-delivery-methods.test.ts` (9), `create-order.test.ts` (3, the first-ever direct test of `createOrder` — previously untested even before this phase, a pre-existing gap this phase partially closed for the specific new logic it added, not a claim of full coverage). Full suite 667/667, `typecheck`/`lint`/`build` all clean. Live verification used the REAL pre-existing Playwright e2e spec (`checkout.spec.ts`, unchanged — the new selector defaults to the first active delivery method, so the existing test flow still works with zero modification) rather than manual click-through, since the Browser pane's interactive ref-based tools require the pane to be actually displayed/foregrounded, which it wasn't this pass (same constraint already documented in memory). Ran it on both configured browsers (`desktop-chromium`, `mobile-safari`) — both green — then queried the real `Order` row it created: `deliveryMethodId`/`deliveryMethodNamePl: "Kurier"` correctly recorded, `shippingGrosze: 0` correctly reflecting the free-shipping threshold being cleared by the real order total. Also confirmed `/koszyk/zamowienie` and `/panel/dostawa` render the real seeded methods/prices via server-rendered `fetch` output.

## 9z47. Phase 6 shipped: payment method CRUD + real checkout wiring, no fake payment

Continuing the plan from §9z42–§9z46. The clarifying answer that shaped this whole phase, from before any code was written: *"Real DB-driven config + honest unconnected state."* Every design decision below traces back to that sentence.

- New `PaymentMethodConfig` model. `provider` reuses the existing `PaymentMethod` enum (extended additively with `PRZELEWY24`/`CARD`/`PAYPAL`) rather than introducing a parallel enum — the same "extend, don't duplicate" instruction (§18) already applied in phases 4/5. `isConnected: Boolean` is the real gate: a row can be a genuine, staff-visible, fully-formed database record while still never being selectable at checkout, because those are deliberately different facts.
- **The most consequential decision this phase**: `isConnected` is not exposed anywhere in the admin form — no checkbox, no toggle, nothing. Every other boolean in this codebase's admin panel (`isActive` on a dozen entities) is a real toggle a staff member can flip. This one deliberately isn't, because flipping it would let an admin "enable" Przelewy24 with zero real payment integration behind it — exactly the "no fake payment" rule, enforced by the UI's own shape rather than by a comment asking someone not to do it. The only way `isConnected` ever becomes `true` is a real code change (seed data today; a real `PaymentProvider` implementation, still unbuilt per `docs/ARCHITECTURE.md` §14, in the future).
- `PaymentStatus` extended additively (`PENDING`/`FAILED`/`CANCELLED`) — same "no existing value removed or repurposed" discipline as every other enum extension this session. `Order.paymentMethodConfigId` added purely for traceability; the existing `paymentMethod` enum column and every real tested action that reads it (`applyMarkOrderPaid`'s `paymentMethod !== 'BANK_TRANSFER'` gate, specifically) are completely untouched.
- `CheckoutForm.tsx`'s payment radios became DB-driven, sourced from `listActivePaymentMethods()` — already filtered to `isConnected: true` in the repository itself, so an unconnected provider is architecturally incapable of reaching the customer, not just hidden by a UI condition that could be forgotten somewhere else. `createOrder` re-validates the chosen config (active + connected) before anything else and derives the stored `paymentMethod` from the real row, mirroring exactly the delivery-method pattern phase 5 established — the two now read as one consistent "look it up, validate it, derive from it" shape rather than two different one-off implementations.
- Seeded three real rows: the two payment methods checkout has always actually supported, now DB-driven instead of hardcoded, plus a real Przelewy24 row with `isConnected: false` — visible in `/panel/platnosci`, provably unreachable at checkout (verified directly, see below), the concrete, working version of "honest unconnected state."

### Verified

Extended `tests/integration/create-order.test.ts` with 3 payment-validation cases (non-existent config, deactivated config, real-but-unconnected config — all three rejected identically with `PAYMENT_METHOD_INVALID`) and added `tests/integration/admin-payment-methods.test.ts` (8 cases, including one that explicitly proves a freshly-created config never appears in the real public listing while unconnected, and one that simulates a real integration going live via direct DB update — never via the form — to prove deactivation still correctly removes a *connected* method from checkout). Full suite 678/678, `typecheck`/`lint`/`build` all clean.

Live verification, in order: (1) the real, unchanged Playwright `checkout.spec.ts` passes end-to-end; (2) the resulting real `Order` row shows the correct `paymentMethodConfigId` and a `paymentMethod` correctly derived as `BANK_TRANSFER`; (3) `/panel/platnosci` lists all three real rows, including Przelewy24; (4) the Przelewy24 admin detail page renders a real read-only "Niepołączona" chip with no `isConnected` form field anywhere in the rendered HTML (checked directly, not assumed); (5) seeded a real cart item and confirmed `/koszyk/zamowienie`'s rendered HTML contains "Przelew bankowy" and "Ustalenie indywidualne" but **not** "Przelewy24" — the concrete proof that a real, active, admin-visible payment method still never reaches the customer while unconnected.

## 9z48. Phase 7 shipped: shipment tracking — real, manual, honestly not-live

Continuing the plan from §9z42–§9z47.

- New `Shipment` model, genuinely one-to-one with `Order` — that's the real shape of the business (one order, one package), not a convenience shortcut that will need revisiting later. Fields split deliberately: `internalNotesPl`/`issueResolutionPl` are staff-only, everything else can reach the customer. Enforced by TYPE, not just by which template happens to render which field — the customer-facing repository type (`OrderShipmentView` in `orders.ts`) is its own `Pick`-style selection, so a staff-only field literally cannot leak through that code path by a future accidental addition to a shared render.
- Admin side is one nested editor (`ShipmentEditor.tsx`) on the existing order detail page, not a new top-level list/CRUD section — there's nothing to list, a shipment only ever exists in the context of its one order. `applyUpsertShipment` upserts (create the first time, update after), same shape as every other "one row per parent" pattern in this codebase.
- The customer-facing half reused the *existing* sharing the guest confirmation page and the logged-in order-history page already had (`OrderConfirmationView`, `findOrderForUser`/`findOrderForConfirmation`, the `OrderSummary` component) — added `shipment` to that one shared view type and one shared new `OrderShipmentInfo` primitive, rather than building two separate shipment displays for two pages that already shared everything else.
- A real, if minor, structural fix surfaced by that change: `OrderSummary.tsx`'s prop type was literally `order: OrderConfirmationView`, and the admin order page passes its own, differently-shaped `AdminOrderView` to the same component (works today only because TypeScript's structural typing tolerated it — `AdminOrderView` happened to be a superset of the fields `OrderSummary` reads). Adding a new required `shipment` field to `OrderConfirmationView` would have silently required `AdminOrderView` to grow a matching field it has no use for. Fixed properly: narrowed `OrderSummary`'s prop type to a `Pick` of only the four fields it actually renders, decoupling it from either caller's full view type — the kind of coupling that's easy to miss until a change exactly like this one exposes it.
- Every customer-facing surface carries an explicit, visible "updated manually, not live" disclaimer whenever a shipment exists, and an honest "not yet prepared" state before one does — no invented "in transit" default, no fake progress bar. No carrier API is integrated anywhere in this project (§9/§15), and nothing here pretends otherwise.
...
## 9z51. Post-P9 round 8: cart UI/UX rebuild, quantity cap, navbar UX, real Przelewy24 + pickup-point plumbing, collection photography (2026-08-29)

Owner feedback covered six real things at once: the cart still looked like raw HTML, no upper bound existed on line-item quantity, the "Duplikuj" button needed a real redesign, Przelewy24/carrier pickup-point selection needed real (if honestly unconnected) plumbing, the navbar needed icons/chevrons/a live cart badge and a "Kolekcje" dropdown, and four real photos the owner had generated (`ChatGPT*.png`, in their own Downloads folder) were ready to use for the collections round 7 seeded empty.

- **The quantity cap is the one item worth calling out as a real safety fix, not just polish**: nothing anywhere previously bounded `CartItem.quantity` — `addToCart`/`updateCartItemQuantity` both accepted any positive integer, so a customer typing `10000` (or a direct POST bypassing the UI) really would have gone through. `domain/cart/quantity.ts`'s `clampCartQuantity` is now the single choke point every mutation path routes through, tested at the unit level (the literal 10 000-unit scenario) and end-to-end in a real browser (`cart.spec.ts`, with `formNoValidate` on the "Aktualizuj" button specifically so the browser's own HTML5 `max` validation can't accidentally make the test (or a real bypass attempt) look blocked when it's actually the server doing the clamping).
- **A real, live bug caught and fixed mid-session**: the first cart-page rewrite kept `koszyk/page.tsx` itself as the MUI-rendering Server Component and used `<Button component={Link}>` directly in it. Two independent problems, both confirmed live: (1) `ARCHITECTURE.md` §2.1's own lint rule forbids `@mui/material` directly inside `(shop)`/`(marketing)` Server Components — `biome.json`'s override caught it immediately; (2) even after moving the MUI rendering into `ui/islands/cart/CartContents.tsx`, leaving that file as a plain Server Component (no `'use client'`) crashed at runtime with "Functions cannot be passed directly to Client Components" — passing `next/link`'s `Link` as a `component` prop into MUI's `Button` from Server-Component scope isn't the same as rendering it directly as JSX; it needs to be a real value already in *client* scope. Every other `component={Link}` use already in this codebase (`PatternsGallery.tsx`, `AccountNav.tsx`, ...) turned out to already be inside a `'use client'` file for exactly this reason — this session just hadn't hit the failure mode before. Fixed by adding `'use client'` to `CartContents.tsx` (no hooks needed — MUI's own components were already Client Components regardless).
- **Przelewy24 and the pickup-point picker are both real code, not stubs that only look real**: `server/services/payment/przelewy24.ts` implements P24's actual `/transaction/register` request shape and SHA384 signing algorithm; `server/delivery/pickup-points.ts` is a real (if honestly static, not live-API-backed) searchable dataset, wired all the way through `CheckoutForm.tsx` → `submitCheckout` → `createOrder`, which re-validates the chosen point against the same dataset server-side rather than trusting the client. Both are unit-tested without ever touching a network or a real merchant account. Przelewy24 stays `isConnected: false` in the seed on purpose — per that column's own schema comment, only real code AND real credentials together earn `true`; the code is now real, the credentials aren't, so the honest state is unchanged until someone adds four env vars.
- **The four owner-supplied images were real files on disk this time**, unlike an earlier round in this same session where the owner pasted images directly into chat and there was genuinely no tool available to extract them — worth noting the difference explicitly, since it's the reason this round could actually finish that specific ask where the earlier one couldn't.

### Verified

`typecheck`/`lint`/`test` (743/743, +24 new) /`build` all clean. New migration applied to both dev and test databases, Prisma client regenerated. Full Playwright e2e: 25/30 green on the first fully-parallel run; the 5 failures were re-run serially (`--workers=1`) and all passed, confirming the project's own pre-existing "destination stream closed early" contention pattern (already documented in this file from earlier rounds) rather than a real regression — not a new flakiness class introduced this round. Live-verified extensively in the Browser pane: cart quantity stepper + navbar cart badge updating together in real time after a click, the "Kolekcje" navbar dropdown listing all six real collections, and the Paczkomat picker filtering to Kraków-only results and recording a real selected point id in the form's hidden input.

## 9z52. Post-P9 round 9: real weight-based shipping, collection hero image bug, cart/checkout redesign, required phone, courier notes (2026-08-29)

Direct, pointed feedback on round 8's own work — including one real bug (the collection detail page never rendered its own `imageUrl`) and one genuine "you're faking it" call-out (the pickup-point picker's static dataset). Two clarifying questions were asked and answered up front, and the answers mattered: the owner explicitly said "you are not allowed to lie" about sourcing carrier pricing, then handed over InPost's and DPD's real cennik URLs and — when `WebFetch` got blocked by DPD's own bot protection — pasted the real official DPD Pickup price table into chat by hand rather than let the block become an excuse to guess.

- **The honesty constraint shaped the whole implementation, not just the pricing numbers**: real weight-tier pricing needed a real per-configuration weight, which needed a real material density — sourced from standard published wood/porcelain reference figures (cited in `prisma/seed.ts`), explicitly NOT the same category of thing as inventing a courier's price, but held to the same "cite the real source" discipline anyway. GEIS got the same treatment in the other direction: real search across five real courier-comparison sites, genuinely found no published static price list anywhere (every source only offers an interactive per-shipment quote form) — seeded as a real, staff-visible, `isActive: false` row with a comment saying exactly that, rather than inventing numbers to fill the gap or silently dropping the carrier the owner asked about.
- **A real geometry bug worth naming**: the first pass at InPost locker-fit feasibility checked "does this item fit the LARGEST available locker" as one global gate, then picked price purely by weight — which could hand back XS pricing for an item that only actually fits size C. Caught before landing anywhere by simply reasoning through InPost's real published dimensions (their door/compartment opening is genuinely fixed-size, only depth varies) — rewritten so `evaluateDeliveryMethod` checks fit AND weight together, per tier, cheapest first.
- **The e2e suite caught a real regression, and the fix was to fix the tests, not loosen the requirement**: making `Order.phone` required broke three checkout specs that never filled in a phone number — confirmed by re-running the affected specs both fully parallel and serially, isolating it to a genuine new required field rather than the project's own pre-existing "destination stream closed early" contention flakiness (which showed up too, on different, unrelated tests, both before and after the fix).
- **"Payment just sends an email, DNO"**: this round improved everything genuinely improvable without a real payment gateway (a real two-column checkout, real weight-based pricing, a prominent order-status banner, color-coded status chips in order history) but did not — could not, honestly — make bank-transfer/contact-arranged checkout feel like a real hosted-payment redirect. Said so plainly rather than papering over it with a fake "processing" animation.
